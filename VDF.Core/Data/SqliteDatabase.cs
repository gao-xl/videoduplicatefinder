...
namespace VDF.Core.Data {
	...
	sealed class SqliteDatabase : IDisposable {
		...
		// ---- Serialization helpers for ConcurrentDictionary<double, byte[]?> ----

		static byte[]? SerializeGrayBytes(ConcurrentDictionary<double, byte[]?> grayBytes) {
			if (grayBytes.Count == 0) return null;
			return MemoryPackSerializer.Serialize(grayBytes);
		}

		static ConcurrentDictionary<double, byte[]?> DeserializeGrayBytes(byte[] data) {
			return MemoryPackSerializer.Deserialize<ConcurrentDictionary<double, byte[]?>>(data)
				?? new ConcurrentDictionary<double, byte[]?>();
		}

		static byte[]? SerializePHashes(ConcurrentDictionary<double, ulong?> pHashes) {
			if (pHashes.Count == 0) return null;
			return MemoryPackSerializer.Serialize(pHashes);
		}

		static ConcurrentDictionary<double, ulong?> DeserializePHashes(byte[] data) {
			return MemoryPackSerializer.Deserialize<ConcurrentDictionary<double, ulong?>>(data)
				?? new ConcurrentDictionary<double, ulong?>();
		}
